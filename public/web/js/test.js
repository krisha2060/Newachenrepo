document.addEventListener('DOMContentLoaded', function() {
  ...
  function updateTotalPrice() {
    ...
    const kidsItems = document.getElementById('kidsItemsDisplayInput')?.value || ''; // human-readable names
    ...
    if (packageId) {
        selectedInfo.innerHTML = `
            <span class="info-label">
                ...
                ${kidsTotal ? `
                <br><br><strong style="color:#74ac43;">+ Kids Package:</strong> ${kidsCount} kids × $18
    ...
    const kidsItemsDisplay = get('kidsItemsDisplayInput'); // human-readable e.g. "Chicken Fried Rice, Veg Chowmin"
...
function initMobileSwipers() {
   ...
   // ═══════════════════════════════════════════════════════════
//  KIDS PACKAGE ADD-ON MODULE - NO HARDCODED VALUES
// ═══════════════════════════════════════════════════════════

(function () {
    // ALL values come from window (which gets data from database)
    ...
    // If no kids package data, don't initialize
    ...
    // Update modal subtitle with actual price from database
   ...
   function buildOrStep() {
    ...
    orOpts.innerHTML = '';
    kOrChoiceConfirmed = false;  // ← ADD THIS
    kOrChoice = null;            // ← ADD THIS
    
    kOrOptions.forEach((opt, i) => {
        const pill = document.createElement('div');
        pill.className = 'kids-or-pill' + (i === 0 ? ' active' : '');
        pill.textContent = opt;
        pill.addEventListener('click', () => {
            document.querySelectorAll('.kids-or-pill').forEach(p => p.classList.remove('active'));
            pill.classList.add('active');
            kOrChoice = opt;
            kOrChoiceConfirmed = true;  // ← ADD THIS
        });
    ...
    function goToStep(s) {
        ...
        btnBack.style.display = (s > 0 && !(s === 1 && !kHasOr)) ? 'inline-flex' : 'none';
 ...
 function advanceStep() {
    ...
    // STEP 0 → select items
    if (kStep === 0) {
        if (!kSelected.length || kSelected.length < 2) {
    ...
    // STEP 2 → kids count
        ...
        kCount = parseInt(document.getElementById("kidsCountDisplay").value) || 0;

        if (!kCount || kCount < 10) {
            alert('Minimum 10 kids required');
            return;
        }
 ...
 function confirmKids() {
    ...
    inKidsItems.value = JSON.stringify(finalItems); // ["Chicken Nuggets (4pcs) & Chips", "Veg Chowmein"] — names!
